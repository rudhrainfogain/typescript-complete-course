...
const multiplyNumbersAndReturnProductArrow= (number1: number, number2: number) =>  number1*number2;
...
const countdown1 = (start: number = 10, end: number = start-5) :void => {
    console.log('start initially was '+start)
...
function makeArray(val1:number,val2:number){
    return [val1,val2];
}
...
function makeArrayRest1(name: string, ...args: number[]){
...
function printInfo1(...info: [string, number]) {
...
const myData={name1: 'Rudhra', age:28};
//if i have to print the values in this array i have to do something like
const myNamee=myData.name1;
const myAgee=myData.age;
console.log('my details are '+ myNamee+ ' and '+ myAgee);

//using destructuring i can write same as

const {name1,age}=myData;
...
const { name1: name3,age: age3}=myData;
...
const userName='rudhra';
const complexString= 'this is a heading '+' my name is '+ userName+' i am so cool ';
...
const templateLiteralComplexString=`this is a heading 
my name is ${userName} 
i am so cool`;
